...
class MonitorNode(Node):
    def __init__(self) -> None:
        ...
        self.latest = {
            ...
            'imu_mag_y': 0.0,
            'imu_mag_z': 0.0,
            'tf_map_to_imu_available': False,
            'tf_parent_frame': 'map',
            'tf_child_frame': 'imu_link',
            'tf_tx': 0.0,
            'tf_ty': 0.0,
            'tf_tz': 0.0,
            'tf_qx': 0.0,
        ...
        self.create_subscription(Imu, '/imu/data', self.on_imu_filtered, 10)
    ...
    def on_imu_filtered(self, msg: Imu) -> None:
        qx = float(msg.orientation.x)
        qy = float(msg.orientation.y)
        qz = float(msg.orientation.z)
        qw = float(msg.orientation.w)
        roll, pitch, yaw = self._quaternion_to_rpy(qx, qy, qz, qw)

        self.latest['imu_orientation_x'] = qx
        self.latest['imu_orientation_y'] = qy
        self.latest['imu_orientation_z'] = qz
        self.latest['imu_orientation_w'] = qw
        self.latest['imu_roll_rad'] = roll
        self.latest['imu_pitch_rad'] = pitch
        self.latest['imu_yaw_rad'] = yaw
        self.latest['imu_frame_id'] = msg.header.frame_id if msg.header.frame_id else 'imu_link'

    def on_tf(self, msg: TFMessage) -> None:
        for t in msg.transforms:
            if t.header.frame_id == 'map' and t.child_frame_id == 'imu_link':
                self.latest['tf_map_to_imu_available'] = True
                self.latest['tf_parent_frame'] = t.header.frame_id
                self.latest['tf_child_frame'] = t.child_frame_id
                self.latest['tf_tx'] = float(t.transform.translation.x)
                self.latest['tf_ty'] = float(t.transform.translation.y)
                self.latest['tf_tz'] = float(t.transform.translation.z)
                self.latest['tf_qx'] = float(t.transform.rotation.x)
                self.latest['tf_qy'] = float(t.transform.rotation.y)
                self.latest['tf_qz'] = float(t.transform.rotation.z)
                self.latest['tf_qw'] = float(t.transform.rotation.w)
                self.latest['tf_stamp_sec'] = int(t.header.stamp.sec)
                self.latest['tf_stamp_nanosec'] = int(t.header.stamp.nanosec)
                return
    ...
    def _build_status_json(self) -> str:
        payload = {
            ...
            'imu': {
                'frame_id': str(self.latest.get('imu_frame_id', 'imu_link')),
                'orientation': {
                    'x': self._safe_number(self.latest.get('imu_orientation_x', 0.0), 0.0),
                    'y': self._safe_number(self.latest.get('imu_orientation_y', 0.0), 0.0),
                    'z': self._safe_number(self.latest.get('imu_orientation_z', 0.0), 0.0),
                    'w': self._safe_number(self.latest.get('imu_orientation_w', 1.0), 1.0),
                },
                'rpy_rad': {
                    'roll': self._safe_number(self.latest.get('imu_roll_rad', 0.0), 0.0),
                    'pitch': self._safe_number(self.latest.get('imu_pitch_rad', 0.0), 0.0),
                    'yaw': self._safe_number(self.latest.get('imu_yaw_rad', 0.0), 0.0),
                },
                'accel': {
            ...
            'tf': {
                'map_to_imu': {
                    'available': bool(self.latest.get('tf_map_to_imu_available', False)),
                    'parent_frame': str(self.latest.get('tf_parent_frame', 'map')),
                    'child_frame': str(self.latest.get('tf_child_frame', 'imu_link')),
                    'translation': {
                        'x': self._safe_number(self.latest.get('tf_tx', 0.0), 0.0),
                        'y': self._safe_number(self.latest.get('tf_ty', 0.0), 0.0),
                        'z': self._safe_number(self.latest.get('tf_tz', 0.0), 0.0),
                    },
                    'rotation': {
                        'x': self._safe_number(self.latest.get('tf_qx', 0.0), 0.0),
                        'y': self._safe_number(self.latest.get('tf_qy', 0.0), 0.0),
                        'z': self._safe_number(self.latest.get('tf_qz', 0.0), 0.0),
                        'w': self._safe_number(self.latest.get('tf_qw', 1.0), 1.0),
                    },
                    'stamp': {
                        'sec': self.latest.get('tf_stamp_sec', None),
                        'nanosec': self.latest.get('tf_stamp_nanosec', None),
                    },
                },
            },
            'robot': {
