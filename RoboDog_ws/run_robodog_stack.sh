...
detect_i2c_devices() {
  ...
  local mpu_status="NOT FOUND"
    ...
    # Check for AHT10 at 0x38
    if echo "$scan_output" | grep -qE "^\s+30:\s+.*\s+38" || \
      echo "$scan_output" | grep -q "0x38"; then
    aht10_status="FOUND"
  fi
...
echo "[0/11] Preparing permissions and I2C interface..."
...
echo "[1/11] Detecting I2C hardware devices..."
detect_i2c_devices || true

echo "[2/11] Resetting I2C bus..."
# Unbind and rebind I2C devices to clear stuck state
if [[ -e /sys/bus/i2c/drivers/at24/0-0038 ]]; then
  sudo_run bash -c "echo '0-0038' > /sys/bus/i2c/drivers/at24/unbind" 2>/dev/null || true
fi
if [[ -e /sys/bus/i2c/devices/i2c-1 ]]; then
  sudo_run i2cset -y 1 0x68 0x6B 0x80 2>/dev/null || true  # Reset MPU9250 (PWR_MGMT_1 DEVICE_RESET)
  sleep 1
fi
if [[ -e /sys/bus/i2c/devices/i2c-1 ]]; then
  sudo_run i2cset -y 1 0x38 0x3E 0xBA 2>/dev/null || true  # Reset AHT10 (soft reset)
  sleep 1
fi
echo "  - I2C devices reset"

echo "[3/11] Killing old processes..."
sudo_run killall -9 \
  rosbridge_websocket \
  web_video_server \
  mpu9250_node \
...
echo "[4/11] Cleaning shared memory and temp camera files..."
...
echo "[5/11] Starting main launch stack..."
nohup ros2 launch robodog_bringup web_interface.launch.py > "$LOG_DIR/web_interface.log" 2>&1 &
sleep 4

echo "[6/11] Ensuring rosbridge is running on port 9090..."
...
echo "[7/11] Ensuring web_video_server is running on port 8080..."
...
echo "[9/11] Sensor topic publisher checks:"
...
echo "[10/11] I2C device verification:"
detect_i2c_devices || true

echo "[11/11] WebSocket status_json sample check:"
python3 - <<'PY' || true
import json
